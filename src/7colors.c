 ...
 */
void
nuovo_click_event(GtkWidget * widget, gpointer data)
{
 ...
 */
void
premuto_colore(GtkWidget * widget, gpointer colore)
{
    // abilito bottoni
...
main(int argc, char *argv[])
{
    ...
    gtk_box_pack_start(GTK_BOX(contenitore1), contenitore2, FALSE, FALSE,
		       0);
    ...
    gtk_box_pack_start(GTK_BOX(contenitore1), areadisegno, FALSE, FALSE,
		       0);
    ...
    rombo[1] =
	gdk_pixmap_create_from_xpm_d(areadisegno->window, &maschera,
				     &areadisegno->
				     style->bg[GTK_STATE_NORMAL],
    ...
    larghezza_pixel =
	larghezza_tab * larghezza_rombo + larghezza_rombo / 2;
	...
	gtk_box_pack_start(GTK_BOX(contenitore3), pixmapwid, FALSE, FALSE,
			   0);
	gtk_widget_show(contenitore3);
	gtk_container_add(GTK_CONTAINER(bottonecol[i]), contenitore3);
	gtk_box_pack_start(GTK_BOX(contenitore2), bottonecol[i], TRUE,
			   TRUE, 10);
 ...
 */
int
leggi_config(int argc, char *argv[])
{
    int i, n;
 ...
 */
void
disegna(int x, int y, short int col)
{
    GdkRectangle update_rect;

    gdk_gc_set_clip_origin(miogc,
			   x * larghezza_rombo +
			   ((y + 1) % 2) * larghezza_rombo / 2,
			   y * altezza_rombo / 2);
 ...
 */
int
colora(int x, int y, short int old_col, short int new_col)
{
    int k = 0;
	...
	if (tab[x][y].col == old_col) {
	    ...
	    tab[x][y].segno = 1;
	    if (y % 2 == 0)
		k = colora(x, y - 1, old_col, new_col) + colora(x, y + 1,
								old_col,
								new_col) +
		    colora(x + 1, y - 1, old_col, new_col) + colora(x + 1,
								    y + 1,
								    old_col,
								    new_col)
		    + 1;
		...
		tab[x][y].segno = 1;
		if (y % 2 == 0)
		    k = espandi(x, y - 1, new_col) + espandi(x, y + 1,
							     new_col) +
			espandi(x + 1, y - 1, new_col) + espandi(x + 1,
								 y + 1,
								 new_col) +
			1;
		else
 ...
 */
int
espandi(int x, int y, short int col)
{
    int k = 0;
	...
	if (tab[x][y].col == col && tab[x][y].segno == 0) {
		...
		k = espandi(x, y - 1, col) + espandi(x, y + 1, col) +
		    espandi(x + 1, y - 1, col) + espandi(x + 1, y + 1,
							 col) + 1;
	    else
		k = espandi(x, y - 1, col) + espandi(x, y + 1, col) +
		    espandi(x - 1, y - 1, col) + espandi(x - 1, y + 1,
 ...
 */
int
guadagno(int x, int y, short int old_col, short int new_col)
{
    int k = 0;
	...
	if (tab[x][y].col == old_col && tab[x][y].segno == 0) {
	    tab[x][y].segno = 1;
	    if (y % 2 == 0)
		k = guadagno(x, y - 1, old_col, new_col) + guadagno(x,
								    y + 1,
								    old_col,
								    new_col)
		    + guadagno(x + 1, y - 1, old_col,
			       new_col) + guadagno(x + 1, y + 1, old_col,
						   new_col) + 1;
	    else
		k = guadagno(x, y - 1, old_col, new_col) + guadagno(x,
								    y + 1,
 ...
 */
void
scrivi_perc(short int giocatore)
{
    char perc[10];
